...
<body>
	...
	<%
		...
		//요청객체에서 요청파라미터값 조회하기
		//부서목록페이지에서 detail.jsp를 요청할 때 전달한 부서아이디를 조회하기
		int id = Integer.parseInt(request.getParameter("deptId")); //문자로 되어있는걸 숫자로 바꿔주는것 parseInt "10"->10
		
		//Departments 테이블에 대한 CRUD기능을 제공하는 DepartmentDao객체 생성하기
		DepartmentDao departmentDao = new DepartmentDao();
		//부서기본정보를 반환하는 getDepartmentDto(부서아이디)를 실행해서 부서 기본정보 획득
		//DepartmentDto는 부서정보, 부서관리자정보, 소재지정보, 소속사원숫자를 저장하는 DTO객체다.
		DepartmentDto departmentDto = departmentDao.getDepartmentDto(id);
		
	%>
	<h3>부서 기본 정보</h3>
	<dl>
		<dt>부서아이디</dt><dd><%=id %></dd>
	...
	<p>
		<a href="modifyform.jsp?deptId=<%=id%>"><strong>부서정보 수정</strong></a>
	<%
		if(departmentDto.getEmpCount() == 0){
	%>
		<a href="remove.jsp?deptId=<%=id%>"><strong>부서정보 삭제</strong></a>
	<%
		}
	%>
	</p>
	...
	<%
		...
		// EmployeeDao객체의 getEmployeesByDepartmentId(부서아이디) 메소드를 실행해서
		// 해당 부서에 소속된 모든 사원정보를 반환받는다.
		List<Employee> employees = employeeDao.getEmployeeByDepartmentId(id);
	%>
	<h3>소속사원 정보</h3>
	<table border="1" style="width: 80%">
		...
		%>
			<tr>
				<td colspan="5" style="text-align:center;">소속된 사원이 존재하지 않습니다.</td>
			</tr>		
		<%		
			} else{
