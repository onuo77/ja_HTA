...
public class AppReview {
	...
	private static String numberToCurrency(int number) {
		return AppReview.numberFormat.format(number);
	}
	...
	private static void printProduct(Product[] products) {
		for(Product item : products) {
			...
			System.out.println("상품가격 : " + AppReview.numberToCurrency(item.getPrice()));
...
//			- 총적립포인트는 상품포인트*구매수량의 총합이다.
		...
		while(true) {
			...
			if(menuNo == 1) {
				...
				AppReview.printProduct(products);
				...
				AppReview.buyProductByNo(products, orders, isExist, buyProductAmount, buyProductNo, position);
...
//				}
				...
				AppReview.printBuyProductInfo(orders);
