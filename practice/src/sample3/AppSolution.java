		...
		while(true) {
			...
			if(menuNo == 1) {
				...
				for (Product product : products) {
					AppSolution.printProduct(product);
				}
				...
				if(category.isBlank()) {
					...
					for (Product product : products) {
						if(product.getCategory().equals(category)) { //카테고리명이 일치하는 경우
							isExist = true;
							AppSolution.printProduct(product);
						}
					}
				...
				System.out.println("[ 상품명으로 상품 출력 ]");
				System.out.print("찾으시는 상품명을 입력하세요. : ");
				String name = sc.next();
